        ...
        for (int i = 0; i < houses.length; i++) {       //soronként haladunk
            ...
            for (int j = 0; j < houses[i].length; j++) {        //oszlopat nézve
                if (houses[i][j] == 0 || j == houses[i].length-1) {   /*  ha 0-hoz ér a lépés, az azt jelenti, hogy ott
                    ...
                    int hanyEmeletes = j;
                    if (j == houses[i].length-1 && houses[i][j] == 1 ) {        /* abban az esetben, ha elértük a
                    legfelső szintet ami 5-1 ,tehát 4, ÉS emelet! adjunk hozzá az emelethez 1-et, így lesz 5. emeletünk is.*/
                        hanyEmeletes++;
                    }
                    if (hanyEmeletes % 2 == 0) {
